defmodule GreenManTavernWeb.RackComponent do
  ...
  def update(assigns, socket) do
    ...
    # Fetch context based on view root
    root_id = socket.assigns.current_view_root_id
    devices = Rack.list_devices(root_id)
    ...
    socket = push_event(socket, "update_cables", %{cables: cables})
      ...
      <div class="rack-container flex-1 h-full relative overflow-y-auto flex flex-col w-full bg-[#f0f0f0]"
        ...
        <div class="w-full mb-4 flex justify-between items-center p-2 bg-white border-b border-[#ccc] sticky top-0 z-30">
          <div class="flex items-center gap-4">
            ...
            <% else %>
              <span class="font-bold text-gray-700">Main Rack</span>
            <% end %>
          </div>
        ...
        <div class="flex flex-1 relative">
              ...
              <%= for input <- (@current_view_device.settings["inputs"] || []) do %>
                <div class="flex flex-col items-center group/jack relative">
                  <div class={"jack rack-jack w-8 h-8 rounded-full bg-[#ccc] border-4 border-[#999] cursor-pointer relative z-20 transition-colors #{if is_selected?(@patching_state, @current_view_device.id, input["id"]), do: "border-[#00f] shadow-[0_0_10px_#00f]", else: "hover:border-[#666]"}"}
                    ...
                    phx-value-device-id={@current_view_device.id}
                    phx-value-jack-id={input["id"]}
                    phx-target={@myself}>
                    <div class="absolute inset-0 m-auto w-4 h-4 rounded-full bg-[#333]"></div>
                  </div>
                  <span class="text-[10px] font-mono mt-1 text-[#333]"><%= input["name"] %></span>
                </div>
              <% end %>
            </div>
          <% end %>
          ...
          <!-- Right Boundary (System Outputs) -->
          <%= if @current_view_device do %>
            <div class="w-16 bg-[#ddd] border-l border-[#999] flex flex-col items-center py-8 gap-4 shadow-inner z-20">
              ...
              <%= for output <- (@current_view_device.settings["outputs"] || []) do %>
                <div class="flex flex-col items-center group/jack relative">
                  ...
                  <span class="text-[10px] font-mono mt-1 text-[#333]"><%= output["name"] %></span>
                </div>
              <% end %>
            </div>
          <% end %>
  ...
  def handle_event("set_view_root", %{"id" => id_str}, socket) do
    ...
    {:noreply,
     socket
     |> assign(:current_view_root_id, new_root_id)
     |> assign(:current_view_device, current_view_device)
     |> assign(:devices, devices)
     |> assign(:cables, cables)
     |> assign(:selected_devices, MapSet.new()) # Clear selection on view change
  ...
  def handle_event("add_device", %{"type" => "composite", "id" => system_id}, socket) do
    ...
    case GreenManTavern.Rack.RackSystemBuilder.instantiate(system_id, user_id, project_id) do
      {:ok, device} ->
        IO.puts("System instantiated successfully: #{inspect(device.id)}")
        # Refresh devices
        devices = Rack.list_devices()
    ...
    if device do
      ...
      {children, internal_cables} = 
        if device.settings["is_composite"] do
          ...
          IO.puts("DEBUG: Edit Device #{device.id} (Composite). Children: #{length(c)}, Cables: #{length(w)}")
          {c, w}
        else
          {[], []}
        end
      ...
      {:noreply, 
       ...
       |> assign(:editing_device_children, children)
       |> assign(:editing_device_cables, internal_cables)}
    else
      IO.puts("DEBUG: Edit Device failed. Device #{id} not found in assigns.")
      {:noreply, socket}
    end
  end
  ...
  def handle_event("toggle_edit_mode", _params, socket) do
    new_mode = if socket.assigns.edit_mode == :front, do: :back, else: :front
    {:noreply, assign(socket, :edit_mode, new_mode)}
  end
